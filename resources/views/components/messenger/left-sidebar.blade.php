            ...
            @else
                <div class="divide-y divide-gray-100">
                    @foreach($messenger_threads as $messenger_thread)
                        <a href="{{ route('messenger', ['action' => 'thread', 'actionable_id' => $messenger_thread->id]) }}" @class(['text-gray-900 dark:text-gray-300 group flex items-center py-2 px-3 cursor-pointer hover:bg-gray-200', 'bg-gray-200 dark:bg-zinc-500' => ($messenger_thread->id === optional($selected_thread)->id)])>
                            <span class="font-medium text-gray-500 dark:text-gray-300 tracking-wider align-middle" style="margin-top: 2px !important;">
