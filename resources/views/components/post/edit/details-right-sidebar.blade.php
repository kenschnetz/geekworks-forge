                    ...
                    @if(count($attributes) > 0)
                        @foreach($attributes as $index => $post_attribute)
                            <div class="border border-gray-200 mt-2 px-2 py-1" x-data="{value: '', limit: 255 }">
                                <span class="font-bold">{{ $post_attribute->Attribute->name }}:</span>
                                <div class="my-1 border border-purple-800 focus:border-purple-800">
                                    ...
                                    <div class="editable-div px-2 py-1 focus:outline-none" x-init="value = $el.textContent" x-on:input="value = $el.textContent" contenteditable placeholder="Value" x-on:blur="$wire.set('attributes.{{$index}}.value', value)">{{ $post_attribute->value }}</div>
                                </div>
                            </div>
                            @error('attributes.' . $index . '.value') <span class="text-red-600 error italic">{{ $message }}</span> @enderror
                        @endforeach
                    @else
                ...
                <div class="mt-2">
                    @if(count($actions) > 0)
                        @foreach($actions as $index => $post_action)
                            <div class="border border-gray-200 mt-2 px-2 py-1" x-data="{value: '', limit: 255 }">
                                <span class="font-bold">{{ $post_action->Action->name }}:</span>
                                <div class="my-1 border border-purple-800 focus:border-purple-800">
                                    <span class="text-xs italic p-1 float-right" x-text="limit - value?.length" :class="{'text-gray-400':  value?.length <= limit, 'text-red-500':  value?.length > limit }"></span>
                                    <div class="editable-div px-2 py-1 focus:outline-none" x-init="value = $el.textContent" x-on:input="value = $el.textContent" contenteditable placeholder="Value" x-on:blur="$wire.set('actions.{{$index}}.value', value)">{{ $post_action->value }}</div>
                                </div>
                            </div>
                            @error('actions.' . $index . '.value') <span class="text-red-600 error italic">{{ $message }}</span> @enderror
                        @endforeach
                    @else
                        <div class="border border-gray-200 mt-1 p-3">
                            <div class="mt-1 bg-gray-100 rounded w-1/3" style="height: 20px;"></div>
